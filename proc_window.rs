...
struct WindowState {
    ...
    quad_mesh: Mesh,
    transform_buffer: wgpu::Buffer, // Uniform buffer for transformation matrix
    //transform_bind_group: wgpu::BindGroup, // Bind group for the uniform buffer
}
...
impl WindowState {
    ...
    // TODO Move into fn. Setup transforms: Vec<Transform> with positions and colors for all the vertices========================================================
    //Set ArrayFire backend (e.g., CPU/CUDA/OpenCL)
    af::set_backend(af::Backend::CUDA);  // maybe not needed or redundant with main
    ...
    //println!("Num Quads X: {}, Num Quads Y: {}", num_quads_x, num_quads_y);
    //wait_for_enter();
    ...
    // Calculate the position of each quad and set position and color into transforms
    for y in 0..num_quads_y {
        for x in 0..num_quads_x {
            // Calculate the position of the quad in pixels
            let pos_x = x as f32 * quad_size.0;
            let pos_y = y as f32 * quad_size.1;
            
            // Convert pixel coordinates to normalized device coordinates (NDC)
            let ndc_x = (pos_x / self.size.width as f32) * 2.0 - 1.0;
            let ndc_y = 1.0 - (pos_y / self.size.height as f32) * 2.0; // Flip Y axis for top-left origin
            //println!("Posn xform: ({}, {}), NDC X: {}, NDC Y: {}", pos_x, pos_y, ndc_x, ndc_y);                // Create a transformation matrix to move the quad to its position
            
            transforms.push(Transform {
                position: [ndc_x, ndc_y, 0.0, 1.0],
                scale: 1.0,
    ...
    render_pass.draw(0..6, 0..transforms.len() as u32);
    ...
    /*    // setup a loop to draw a bunch of quads
        {// limit scope of move of encoder
            let mut render_pass = encoder.begin_render_pass(&render_pass_descriptor);
            render_pass.set_pipeline(&self.render_pipeline);
            render_pass.set_vertex_buffer(0, self.quad_mesh.vertex_buffer.slice(..));
            //render_pass.set_bind_group(0, &self.transform_bind_group, &[]);
            // Quad size in pixels (assuming each quad is 10x10 pixels)
            let quad_size = DFLT_QUAD_SIZE;

            // Calculate the number of quads horizontally and vertically
            let num_quads_x = (self.size.width as f32 / quad_size.0).ceil() as usize;
            let num_quads_y = (self.size.height as f32 / quad_size.1).ceil() as usize;
            //println!("Num Quads X: {}, Num Quads Y: {}", num_quads_x, num_quads_y);
            //wait_for_enter();

            // Loop through the grid and render each quad
            for y in 0..num_quads_y/3 {
                for x in 0..num_quads_x/3 {
                    // Calculate the position of the quad in pixels
                    let pos_x = x as f32 * quad_size.0;
                    let pos_y = y as f32 * quad_size.1;
                    ...
                    let ndc_x = (pos_x / self.size.width as f32) * 2.0 - 1.0;
                    let ndc_y = 1.0 - (pos_y / self.size.height as f32) * 2.0; // Flip Y axis for top-left origin
                    //println!("Posn: ({}, {}), NDC X: {}, NDC Y: {}", pos_x, pos_y, ndc_x, ndc_y);                // Create a transformation matrix to move the quad to its position
                    
                    let transform = glam::Mat4::from_translation(glam::Vec3::new(ndc_x, ndc_y, 0.0));
                    //println!(" Transform: {:?}",transform.to_cols_array());
        ...
        /*{// limit scope of move of encoder
            ...
            render_pass.set_pipeline(&self.render_pipeline);
            render_pass.set_vertex_buffer(0, self.quad_mesh.vertex_buffer.slice(..));
            //render_pass.set_index_buffer(self.quad_mesh.index_buffer.slice(..), wgpu::IndexFormat::Uint16); // For use with TriangleList and old make_quad
            //render_pass.draw_indexed(0..4,0, 0..1); // For use with TriangleList and old make_quad
            render_pass.draw(0..4, 0..1); // Remove for TriangleList
        }*/
        */
